    ...
    endpoints: (builder) => ({
        getProducts: builder.query<IProduct[], { q?: string; categoryId?: number }>({
            ...
            providesTags: (result, _error, arg) =>
                result ? [...result.map(({ id }) => ({ type: 'Products' as const, id })), 'Products'] : ['Products'],
        }),
        ...
        updateProduct: builder.mutation<IProduct, IProduct>({
            ...
            invalidatesTags: (_result, _error, arg) => [{ type: 'Products', id: arg.id }],
        }),
...
export const { useGetProductsQuery, useGetProductByIdQuery, useDeleteProductByIdMutation, useNewProductMutation, useUpdateProductMutation } =
    productApi;
